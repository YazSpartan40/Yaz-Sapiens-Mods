...
local actionUIOuterButtons = mjrequire "mainThread/ui/actionUIOuterButtons"
...
actionUI.cancelIconOffsetScale = 2.4
...
local function rotatedButtonPosVec3(startPos, angle)
    local cosTheta = math.cos(angle);
    local sinTheta = math.sin(angle);
    return vec3(startPos.x * cosTheta - startPos.y * sinTheta, startPos.x * sinTheta + startPos.y * cosTheta, startPos.z)
end
...
actionUI.iconLocationsForCounts = iconLocationsForCounts

local toolTipZDistance = 6.0
local toolTipXYDistance = 40.0
local toolTipLocationsForCounts = {
    [1] = {
        {
            offset = vec3(0.0,toolTipXYDistance,toolTipZDistance),
    ...
    [3] = {
        ...
        {
            offset = vec3(0.0,0.0,toolTipZDistance) + vec3(0.0,-50.0,0.0) + vec3(40.0,0.0, 0.0),
    ...
    [4] = {
        ...
        {
            offset = vec3(toolTipXYDistance,0.0,toolTipZDistance),
            relativePosition = ViewPosition(MJPositionOuterRight, MJPositionCenter),
            cancelOffset = vec3(0.0,0.0,toolTipZDistance),
    ...
    [5] = {
        ...
        {
            offset = vec3(toolTipXYDistance,0.0,toolTipZDistance) + vec3(0.0,30.0,0.0),
            relativePosition = ViewPosition(MJPositionOuterRight, MJPositionCenter),
            cancelOffset = vec3(0.0,0.0,toolTipZDistance),
            cancelRelativePosition = ViewPosition(MJPositionOuterRight, MJPositionCenter),
            orderTextPlinthOffset = vec3(30.0,0.0,0.0),
        ...
        {
            offset = vec3(toolTipXYDistance,0.0,toolTipZDistance) + vec3(-20.0,-100.0,0.0),
            relativePosition = ViewPosition(MJPositionOuterRight, MJPositionCenter),
            cancelOffset = vec3(0.0,0.0,toolTipZDistance),
            cancelRelativePosition = ViewPosition(MJPositionOuterRight, MJPositionCenter),
            orderTextPlinthOffset = vec3(30.0,0.0,0.0),
    ...
    [6] = {
        ...
        {
            offset = vec3(toolTipXYDistance,0.0,toolTipZDistance) + vec3(-5.0,57.0,0.0),
            relativePosition = ViewPosition(MJPositionOuterRight, MJPositionCenter),
            cancelOffset = vec3(0.0,0.0,toolTipZDistance),
            cancelRelativePosition = ViewPosition(MJPositionOuterRight, MJPositionCenter),
            orderTextPlinthOffset = vec3(30.0,0.0,0.0),
...
local function updateVisuals(wheelSegment, buttonTable)
    if buttonTable.icon then
        ...
        if buttonTable.isDestructive then
            wheelSegment:setModel(model:modelIndexForName(buttonTable.modelName), {
                default = material.types.ui_destructiveBackground_red.index
            })
        else
            wheelSegment:setModel(model:modelIndexForName(buttonTable.modelName), {
        ...
        if disabled then
            ...
            if buttonTable.planInfo.recruitChanceOffset then
                tipText = tipText .. " (".. planHelper:getRecruitChanceOffsetText(buttonTable.planInfo.recruitChanceOffset) .. ")"
            elseif buttonTable.planInfo.banishChanceOffset and buttonTable.planInfo.banishChanceOffset < 2 then
                tipText = tipText .. " (".. planHelper:getRecruitChanceOffsetText(buttonTable.planInfo.banishChanceOffset) .. ")"
            end

            if buttonTable.planTypeIndex and plan.types[buttonTable.planTypeIndex].usesRemoveShortcut then
            ...
            elseif buttonTable.planTypeIndex == plan.types.research.index then
                shortcutMappingKey = "radialMenuInvestigate"
            else
        ...
        if buttonTable.planInfo.hasQueuedPlans then
            uiStandardButton:setOrderMarkerCanComplete(buttonTable.cancelButton, buttonTable.planInfo.anyQueuedPlansCanComplete)
                ...
                if planStates then
                    ...
                    if planStatesForTribe then
                        for j,planState in ipairs(planStatesForTribe) do
                            ...
                            planState.researchTypeIndex == buttonTable.researchTypeIndex and 
                            (buttonTable.planInfo.anyQueuedPlansCanComplete or (not planState.canComplete)) then --todo may need to check objectType and researchType
                                return planState
                            end
                        end
                    end
            ...
            if not buttonTable.cancelButton.userData.hover then
                local planInfoString = nil
                if planStateForText then
                    planInfoString = hubUIUtilities:getPlanTitle(planStateForText)
                end
                if not planInfoString then
                    planInfoString = plan.types[buttonTable.planTypeIndex].inProgress
                end

                local materialToUse = material.types.ok.index
                if not buttonTable.planInfo.anyQueuedPlansCanComplete then
                    materialToUse = material.types.warning.index
                end

                local problemStrings = nil
                if planStateForText then
                    problemStrings = hubUIUtilities:getPlanProblemStrings(planStateForText)
                end
            ...
            if buttonTable.changeAssignedSapienClickFunction then
                ...
                if buttonTable.assignedSapienInfos then
                    local firstSapienInfo = buttonTable.assignedSapienInfos[1]
                    uiStandardButton:setIconModel(buttonTable.changeAssignedSapienButton, nil, nil)
                    uiStandardButton:setObjectIcon(buttonTable.changeAssignedSapienButton, firstSapienInfo)
                    uiToolTip:updateText(buttonTable.changeAssignedSapienButton.userData.backgroundView, locale:get("plan_stop") .. " " .. firstSapienInfo.sharedState.name, nil, false)
                    uiStandardButton:setSecondaryHoverIconModel(buttonTable.changeAssignedSapienButton, "icon_cancel", {
                        default = material.types.red.index
                    })

                    local addButtonSetIfRequired = false
                    if buttonTable.hasMaxSapiensAssigned then
                        addButtonSetIfRequired = true
                    end

                    for i,extraButtonView in ipairs(buttonTable.extraChangeAssignedSapienButtons) do
                        local sapienInfo = buttonTable.assignedSapienInfos[i + 1]
                        if sapienInfo then
                            extraButtonView.hidden = false
                            uiStandardButton:setIconModel(extraButtonView, nil, nil)
                            uiStandardButton:setObjectIcon(extraButtonView, sapienInfo)
                            uiToolTip:updateText(extraButtonView.userData.backgroundView, locale:get("plan_stop") .. " " .. sapienInfo.sharedState.name, nil, false)
                            uiStandardButton:setSecondaryHoverIconModel(extraButtonView, "icon_cancel", {
                                default = material.types.red.index
                            })
                        else
                            if (not addButtonSetIfRequired) then
                                extraButtonView.hidden = false
                                addButtonSetIfRequired = true
                                uiStandardButton:setObjectIcon(extraButtonView, nil)
                                uiStandardButton:setIconModel(extraButtonView, "icon_sapien", nil)
                                uiToolTip:updateText(extraButtonView.userData.backgroundView, locale:get("ui_action_assignSapien"), nil, false)
                                uiStandardButton:setSecondaryHoverIconModel(extraButtonView, nil)
                            else
                                extraButtonView.hidden = true
                            end
                        end
                    end
                    ...
                    uiStandardButton:setSecondaryHoverIconModel(buttonTable.changeAssignedSapienButton, nil)
                    for i,extraButtonView in ipairs(buttonTable.extraChangeAssignedSapienButtons) do
                        extraButtonView.hidden = true
                    end
                end
...
local function updatePlanAvailibility(result)
    ...
    actionUIOuterButtons:updatePlanAvailibility(result)
end
...
local function sendPlan(planTypeIndex, planInfo, objectOrVertIDs, fillConstructionTypeIndex)
        local addInfo = {
...
local cachedAvailibilityResult = nil
local cachedAvailibilityResultObjectID = nil
...
local function updateButtons()
    ...
    if actionUI.selectedObjects or actionUI.selectedVertInfos then
        ...
        if availablePlans then
            ...
            local groupPlanInfosByGroupTypeIndex = {}
            local groupHeaderInfosByGroupTypeIndex = {}

            local consolidatedPlans = {}
            for i,planInfo in ipairs(availablePlans) do
                local planGroupTypeIndex = planInfo.planGroupTypeIndex
                if planGroupTypeIndex then
                    local groupPlanInfos = groupPlanInfosByGroupTypeIndex[planGroupTypeIndex]
                    if groupPlanInfos then
                        table.insert(groupPlanInfos, planInfo)
                        local headerPlanInfo = groupHeaderInfosByGroupTypeIndex[planGroupTypeIndex]
                        headerPlanInfo.hasNonQueuedAvailable = headerPlanInfo.hasNonQueuedAvailable or planInfo.hasNonQueuedAvailable
                        headerPlanInfo.hasQueuedPlans = headerPlanInfo.hasQueuedPlans or planInfo.hasQueuedPlans
                        if planInfo.hasQueuedPlans then
                            headerPlanInfo.anyQueuedPlansCanComplete = headerPlanInfo.anyQueuedPlansCanComplete or planInfo.anyQueuedPlansCanComplete
                        end
                        
                        headerPlanInfo.hasManuallyPrioritizedQueuedPlan = headerPlanInfo.hasManuallyPrioritizedQueuedPlan or planInfo.hasManuallyPrioritizedQueuedPlan
                        headerPlanInfo.unavailableReasonText = headerPlanInfo.unavailableReasonText or planInfo.unavailableReasonText
                    else
                        groupPlanInfos = {}
                        groupPlanInfosByGroupTypeIndex[planGroupTypeIndex] = groupPlanInfos

                        local subPlanInfo = planInfo
                        table.insert(groupPlanInfos, subPlanInfo)

                        local headerPlanInfo = mj:cloneTable(subPlanInfo)
                        groupHeaderInfosByGroupTypeIndex[planGroupTypeIndex] = headerPlanInfo
                        
                        headerPlanInfo.groupPlanInfos = groupPlanInfos
                        headerPlanInfo.planTypeIndex = planGroupTypeIndex

                        headerPlanInfo.disabledPlanTypeIndexes = {} --todo populate initial disabledPlanTypeIndexes either from provided planStates if available or previously used

                        table.insert(consolidatedPlans, headerPlanInfo)
                    end
                else
                    table.insert(consolidatedPlans, planInfo)
                end
            end

            --mj:log("consolidatedPlans:", consolidatedPlans)
            --mj:log("groupHeaderInfosByGroupTypeIndex:", groupHeaderInfosByGroupTypeIndex)


            local skipUnavailableCount = 0
            if #consolidatedPlans > 6 then
                mj:warn("consolidatedPlans > 6:", consolidatedPlans)
                for i,planInfo in ipairs(consolidatedPlans) do
                    mj:log("plan type:", plan.types[planInfo.planTypeIndex])
                end
                skipUnavailableCount = #consolidatedPlans - 6
                --error()
            end
            ...
            for i,planInfo in ipairs(consolidatedPlans) do
                ...
                if not skip then
                    ...
                    local toolTipText = getToolTipText(planTypeIndex, false)
                    ...
                    if plan.types[planTypeIndex].checkCanCompleteForRadialUI then
                    ...
                    if planInfo.hasNonQueuedAvailable then
                        ...
                        buttonFunction = function(wasQuickSwipeAction, hasConfirmed)
                            if (not hasConfirmed) and planInfo.requiresConfirmation then --todo requiresConfirmation assumes banish only
                                if alertPanel:hidden() then
                                    alertPanel:show(actionUI.mainView, locale:get("action_banish"), locale:get("banish_areYouSure", {name = actionUI.baseObject.sharedState.name}), {
                                        {
                                            isDefault = true,
                                            name = locale:get("action_banish"),
                                            action = function()
                                                alertPanel:hide()
                                                buttonFunction(wasQuickSwipeAction, true)
                                            end
                                        },
                                        {
                                            isCancel = true,
                            ...
                            --mj:log("planInfo:", planInfo)
                            if planTypeIndex == plan.types.moveTo.index then
                                actionUI:animateOutForOptionSelected()
                            ...
                            elseif planTypeIndex == plan.types.stop.index then
                                logicInterface:callServerFunction("cancelSapienOrders", {
                                    sapienIDs = objectOrVertIDs,
                                })
                                actionUI:animateOutForOptionSelected()
                            elseif planTypeIndex == plan.types.wait.index then
                                logicInterface:callServerFunction("addWaitOrder", {
                                    sapienIDs = objectOrVertIDs,
                                })
                                actionUI:animateOutForOptionSelected()
                            elseif planTypeIndex == plan.types.clone.index then
                                if inspectUI.baseObjectOrVertInfo then
                                    buildModeInteractUI:showForDuplication(inspectUI.baseObjectOrVertInfo)
                                    actionUI:animateOutForOptionSelected()
                                end
                            elseif planTypeIndex == plan.types.craft.index or 
                            planTypeIndex == plan.types.manageStorage.index or 
                            planTypeIndex == plan.types.manageSapien.index or 
                            planTypeIndex == plan.types.constructWith.index or 
                            planTypeIndex == plan.types.rebuild.index then
                                inspectUI:showInspectPanelForActionUISelectedPlanType(planTypeIndex)
                            elseif planTypeIndex == plan.types.allowUse.index then
                            ...
                            elseif planTypeIndex == plan.types.manageTribeRelations.index then
                                ...
                                actionUI:animateOutForOptionSelected()
                            elseif planTypeIndex == plan.types.startRoute.index then
                                ...
                                actionUI:animateOutForOptionSelected()
                            elseif planTypeIndex == plan.types.attack.index or planTypeIndex == plan.types.defend.index then
                                for j,subPlanInfo in ipairs(planInfo.groupPlanInfos) do
                                    --if not disabled
                                    if subPlanInfo.hasNonQueuedAvailable then
                                        if not actionUIOuterButtons:getPlanDisabled(planTypeIndex, subPlanInfo.planTypeIndex) then
                                            sendPlan(subPlanInfo.planTypeIndex, subPlanInfo, objectOrVertIDs, nil)
                                        end
                                    end
                                end
                                --tribeRelationsUI:show(mainThreadDestination.destinationInfosByID[actionUI.baseObject.sharedState.tribeID], nil, nil, nil, false)
                                ...
                                sendPlan(planTypeIndex, planInfo, objectOrVertIDs, fillConstructionTypeIndex)
                                actionUI:animateOutForOptionSelected()
                            end
                    ...
                    local assignedSapienInfos = nil
                    local assignedCountsByPlanTypeIndex = {}

                    if planInfo.hasQueuedPlans then

                        if planTypeIndex == plan.types.attack.index or planTypeIndex == plan.types.defend.index then
                            cancelClickFunction = function()
                                audio:playUISound(uiCommon.cancelSoundFile)
                                for j,subPlanInfo in ipairs(planInfo.groupPlanInfos) do
                                    logicInterface:callServerFunction("cancelPlans", {
                                        planTypeIndex = subPlanInfo.planTypeIndex,
                                        objectTypeIndex = subPlanInfo.objectTypeIndex,
                                        researchTypeIndex = subPlanInfo.researchTypeIndex,
                                        discoveryCraftableTypeIndex = subPlanInfo.discoveryCraftableTypeIndex,
                                        objectOrVertIDs = objectOrVertIDs,
                                    })
                                end
                            end
                            ...
                            cancelClickFunction = function()
                                ...
                                elseif planTypeIndex == plan.types.wait.index then
                                    ...
                                    local planTypeIndexToUseForCancel = planTypeIndex
                        ...
                        if (not planInfo.cancelIsFollowerOrderQueue) and (planTypeIndex ~= plan.types.wait.index) and (not planInfo.cancelIsElsewhere) then
                            prioritizeFunction = function()
                                local planTypeIndexToUse = planTypeIndex
                        ...
                        if #objectOrVertIDs == 1 and (not planInfo.cancelIsElsewhere) and (planInfo.anyQueuedPlansCanComplete) then
                            ...
                            local function getPlanTypeMatches(orderContext)
                                --mj:log("getPlanTypeMatches:", orderContext, " planTypeIndex:", planTypeIndex)
                                if orderContext.planTypeIndex == planTypeIndex then
                                    return true
                                end

                                if planInfo.groupPlanInfos then
                                    for j,subPlanInfo in ipairs(planInfo.groupPlanInfos) do
                                        if orderContext.planTypeIndex == subPlanInfo.planTypeIndex and orderContext.researchTypeIndex == subPlanInfo.researchTypeIndex then
                                            return true
                                        end
                                    end
                                end
                                    ...
                                    for assignedSapienID,planTypeIndexOrTrue in pairs(assignedSapienIDs) do
                                        ...
                                        if sapienInfo then
                                            ...
                                            if orderQueue and orderQueue[1] then
                                                ...
                                                getPlanTypeMatches(orderContext) and 
                                            ((not planInfo.objectTypeIndex) or (not orderContext.objectTypeIndex) or orderContext.objectTypeIndex == planInfo.objectTypeIndex) then
                                                    ...
                                                    if not assignedSapienInfos then
                                                        assignedSapienInfos = {}
                                                    end
                                                    table.insert(assignedSapienInfos, {
                                                        uniqueID = assignedSapienID,
                                                        objectTypeIndex = gameObject.types.sapien.index,
                                                        sharedState = sapienInfo.sharedState,
                                                    })
                                                    assignedCountsByPlanTypeIndex[orderContext.planTypeIndex] = (assignedCountsByPlanTypeIndex[orderContext.planTypeIndex] or 0) + 1
                                                end
                                            end
                                        end
                                    end
                            ...
                            if not planInfo.sapienAssignButtonShouldBeHidden then
                                changeAssignedSapienClickFunction = function(assignButtonIndex)

                                    local assignedInfo = assignedSapienInfos and assignedSapienInfos[assignButtonIndex]
                                    --mj:log("assignButtonIndex:", assignButtonIndex, " assignedInfo:", assignedInfo)

                                    if assignedInfo then
                                        logicInterface:callServerFunction("cancelAll", {
                                            objectIDs = {assignedInfo.uniqueID},
                                        })
                                    else
                                        actionUI:animateOutForOptionSelected()
                    ...
                    local foundAvailablePlanWithoutMaxAssigned = false
                    for thisPlanTypeIndex, assignedCount in pairs(assignedCountsByPlanTypeIndex) do
                        local maxAssignCount = plan.types[thisPlanTypeIndex].maxAssignCount or 1
                        --mj:log("maxAssignCount:",maxAssignCount, " thisPlanTypeIndex:", thisPlanTypeIndex)
                        if assignedCount < maxAssignCount then
                            foundAvailablePlanWithoutMaxAssigned = true
                        end
                    end

                   -- mj:log("assignedCountsByPlanTypeIndex:", assignedCountsByPlanTypeIndex)
                    --mj:log("foundAvailablePlanWithoutMaxAssigned:", foundAvailablePlanWithoutMaxAssigned)

                    if planInfo.hasExtraOptionsPanel then
                        optionsClickFunction = function()
                            inspectUI:showInspectPanelForActionUIOptionsButton(planTypeIndex)
                        end
                    end
                    ...
                    if planInfo.isDestructiveOverride ~= nil then
                        ...
                        isDestructive = plan.types[planTypeIndex].isDestructive or (planInfo.researchTypeIndex and research.types[planInfo.researchTypeIndex].isDestructive)
                    end
                    ...
                    buttonInfos[buttonIndex] = {
                        ...
                        assignedSapienInfos = assignedSapienInfos,
                        isDestructive = isDestructive,
                        hasMaxSapiensAssigned = (not foundAvailablePlanWithoutMaxAssigned),
                    }
                end
        ...
        logicInterface:callServerFunction("checkPlanAvailability", availabilityRequest, function(result)
            --mj:log("checkPlanAvailability request:", availabilityRequest, " result:", result)
            if thisRequestCounter == planAvailibilityRequestCounter then
                updatePlanAvailibility(result)
                cachedAvailibilityResult = result
                cachedAvailibilityResultObjectID = baseObjectOrVertID
            end
        end)
    else
    ...
    for segmentIndex=1,buttonCount do
        ...
        segmentTable.hasMaxSapiensAssigned = buttonInfos[segmentIndex].hasMaxSapiensAssigned
        segmentTable.assignedSapienInfos = buttonInfos[segmentIndex].assignedSapienInfos
        ...
        if segmentTable.planInfo.isDestructiveOverride ~= nil then
            segmentTable.isDestructive = segmentTable.planInfo.isDestructiveOverride
        else
            segmentTable.isDestructive = plan.types[segmentTable.planTypeIndex].isDestructive or (segmentTable.researchTypeIndex and research.types[segmentTable.researchTypeIndex].isDestructive)
        end
        ...
        uiStandardButton:setClickFunction(segmentTable.changeAssignedSapienButton, function()
            segmentTable.changeAssignedSapienClickFunction(1)
        end)
    ...
    actionUIOuterButtons:update()
    
    if cachedAvailibilityResultObjectID == baseObjectOrVertID then
        updatePlanAvailibility(cachedAvailibilityResult)
    end
...
function actionUI:selectButtonAtIndex(index, shouldAutomateOrOpenOptions)
    ...
    if buttonInfo then
        if buttonInfo.planTypeIndex and plan.types[buttonInfo.planTypeIndex].usesRemoveShortcut then
...
function actionUI:selectDeconstructAction(shouldAutomateOrOpenOptions)
    if actionUI.currentWheelIndex then
        for index=1,actionUI.currentWheelIndex do
            local buttonInfo = buttonInfos[index]
            if buttonInfo and buttonInfo.planTypeIndex and plan.types[buttonInfo.planTypeIndex].usesRemoveShortcut then --(plan.types[buttonInfo.planTypeIndex].isDestructive or (buttonInfo.researchTypeIndex and research.types[buttonInfo.researchTypeIndex].isDestructive)) then
                if buttonInfo.cancelClickFunction then
                    audio:playUISound(uiCommon.failSoundFile)
                    actionUI:animateOutForOptionSelected()
                    return
...
function actionUI:selectInvestigateAction(shouldAutomateOrOpenOptions)
    if actionUI.currentWheelIndex then
        for index=1,actionUI.currentWheelIndex do
            local buttonInfo = buttonInfos[index]
            if buttonInfo and buttonInfo.planTypeIndex == plan.types.research.index then
                if buttonInfo.cancelClickFunction then
                    audio:playUISound(uiCommon.failSoundFile)
                    actionUI:animateOutForOptionSelected()
                    --buttonInfo.cancelClickFunction()
   ...
   -- local updateTimer = 0.0
    ...
    local function addWheelSegment(segmentCount, segmentIndex)
        ...
        wheelSegment.hoverStart = function (mouseLoc)
            if not (buttonTable.hover or buttonTable.disabledHover) then
                if not buttonTable.disabled then
                    ...
                    actionUIOuterButtons:animateOn(wheelSegment)
       ...
       --[[ wheelSegment.click = function()
        ...
        uiToolTip:add(cancelButton.userData.backgroundView, ViewPosition(MJPositionCenter, MJPositionAbove), locale:get("ui_action_stop"), nil, vec3(0.0,0.0,0.0), nil, cancelButton)
        ...
        local currentOrderTextPlinth = ModelView.new(cancelButton)
        buttonTable.currentOrderTextPlinth = currentOrderTextPlinth
        currentOrderTextPlinth.relativeView = cancelButton
        currentOrderTextPlinth.relativePosition = ViewPosition(MJPositionCenter, MJPositionAbove)
        ...
        buttonTable.extraChangeAssignedSapienButtons = {}
        local changeAssignedSapienButtonRelativeView = changeAssignedSapienButton
        for i = 1,7 do
            local nextChangeAssignedSapienButton = uiStandardButton:create(cancelButton, actionUI.planStatusSize * 0.6, uiStandardButton.types.orderMarkerSmall, {
                [material.types.ui_standard.index] = material.types.ok.index,
            })
            buttonTable.extraChangeAssignedSapienButtons[i] = nextChangeAssignedSapienButton
            nextChangeAssignedSapienButton.relativeView = changeAssignedSapienButtonRelativeView
            changeAssignedSapienButtonRelativeView = nextChangeAssignedSapienButton

            local isOnLeft = buttonLocation.offset.x < -0.7

            if isOnLeft then
                nextChangeAssignedSapienButton.relativePosition = ViewPosition(MJPositionCenter, MJPositionBelow)
                nextChangeAssignedSapienButton.baseOffset = vec3(0,8,-0.2)
            else
                nextChangeAssignedSapienButton.relativePosition = ViewPosition(MJPositionOuterRight, MJPositionCenter)
                nextChangeAssignedSapienButton.baseOffset = vec3(-8,0,-0.2)
            end

            uiToolTip:add(nextChangeAssignedSapienButton.userData.backgroundView, toolTipLocation.cancelRelativePosition, locale:get("ui_action_assignDifferentSapien"), nil, toolTipLocation.cancelOffset, nil, nextChangeAssignedSapienButton)

            
            uiStandardButton:setClickFunction(nextChangeAssignedSapienButton, function()
                buttonTable.changeAssignedSapienClickFunction(i + 1)
            end)
        end
    ...
    actionUIOuterButtons:init(gameUI_, hubUI_, world_, actionUI)
...
function actionUI:controllerOptionsButton()
    --[[if (not actionUI.mainView.hidden) and ((not animatingInOrOut) or animatingIn) then --todo
        local wheel = actionUI.wheels[actionUI.currentWheelIndex]
        if wheel.selectedSegment then
            local segment = wheel.segments[wheel.selectedSegment]
            local segmentTable = segment.userData
...
function actionUI:updateButtons()
    updateButtons()
end
...
function actionUI:isAnimatingInOrOut()
    return animatingInOrOut
end
