...
local closeSapiensBySkillTypeByObjectByTribe = {} --used for too distant state
local closeThreatSapiensByObjectByTribe = {}
...
function planManager:init(initObjects)

    serverGOM = initObjects.serverGOM
    serverSapien = initObjects.serverSapien
    serverCraftArea = initObjects.serverCraftArea
    ...
    planLightProbes:init(planManager, serverGOM, serverWorld)
...
local function updateCanCompleteAndSave(object, sharedState, planState, planTribeID, planIndex, skipUpdateOfOrderLimits)
    ...
    --these are some handy little catchers of bugs if you accidentatly set something with an incorrect tribeID
...
local function getCloseThreatSapiensByObjectForTribe(tribeID)
    local closeSapiensByObject = closeThreatSapiensByObjectByTribe[tribeID]
    if not closeSapiensByObject then
        closeSapiensByObject = {}
        closeThreatSapiensByObjectByTribe[tribeID] = closeSapiensByObject
    end
    return closeSapiensByObject
end
...
local function checkSapienHasAbilityForRole(sapien, skillTypeIndex, planStateOrNil)
    if not skillTypeIndex then --this is valid, some plans (eg. banish for local sapiens) don't require any skill/role at all
        return true
    end
    if sapienConstants:getHasLimitedGeneralAbility(sapien.sharedState, skillTypeIndex and skill.types[skillTypeIndex].noCapacityWithLimitedGeneralAbilityIncludesElders) then
...
function planManager:updateProximity(skillTypeIndexOrNil, planObjectID, sapienID, newIsClose) --"too distant" proximity test
...
function planManager:updateThreatProximity(planObjectID, sapienID, newIsClose)
    ...
    if planStatesByTribeID then
        for tribeID,planStates in pairs(planStatesByTribeID) do
            local isNewCloseThreat = newIsClose
            if isNewCloseThreat then
                if not serverTribe:getOtherTribeIsThreatening(tribeID, sapienTribeID) then --todo maybe/probably needs to take sapien state into account, but that requires updating all near by plan states any time any sapien threatening state changes
                    isNewCloseThreat = false
                end
            end
            
            local closeThreatSapiensByObject = getCloseThreatSapiensByObjectForTribe(tribeID)

            local becameThreat = false
            local becameSafe = false
            
            local hasCheckedCloseSapiens = false
            local function checkCloseSapiensIfNeeded()
                if not hasCheckedCloseSapiens then
                    hasCheckedCloseSapiens = true
                    local closeSapiens = closeThreatSapiensByObject[planObjectID]
                    if isNewCloseThreat then
                        if not closeSapiens then
                            closeSapiens = {}
                            closeThreatSapiensByObject[planObjectID] = closeSapiens
                            becameThreat = true
                        end
                        closeSapiens[sapienID] = sapienTribeID --sapienTribeID probably isn't needed, but let's keep it consistent with planManager:updateProximity
                    else
                        if closeSapiens then
                            closeSapiens[sapienID] = nil
            
                            if not next(closeSapiens) then
                                closeThreatSapiensByObject[planObjectID] = nil
                                becameSafe = true
                            end
                        end
                    end
            ...
            for i,thisPlanState in ipairs(planStates) do
                if not plan.types[thisPlanState.planTypeIndex].allowCompletionNearThreats then
                    checkCloseSapiensIfNeeded()
                    if becameThreat then
                        planObject.sharedState:set("planStates", tribeID, i, "closeThreat", sapienID)
                        updateCanCompleteAndSave(planObject, planObject.sharedState, thisPlanState, tribeID, i, false)
                    elseif becameSafe then
                        planObject.sharedState:remove("planStates", tribeID, i, "closeThreat")
                        updateCanCompleteAndSave(planObject, planObject.sharedState, thisPlanState, tribeID, i, false)
                    end
                end
            end
...
function planManager:updateAllThreatProximityObjectsForRecruitOrRelationshipSettingChange(tribeID) --brute force, but should be OK. Faster alternative is a lot of work. maybe should batch this up too and call on an update
    local function checkPlan(planInfo)
        local planObject = serverGOM:getObjectWithID(planInfo.objectID)
        if planObject then
            local sharedState = planObject.sharedState
            ...
            if planStatesByTribeID then
                ...
                if planStates then
                    for i,thisPlanState in ipairs(planStates) do
                        if not plan.types[thisPlanState.planTypeIndex].allowCompletionNearThreats then
                            local foundThreatID = false
                            local allCloseSapiens = serverGOM:getGameObjectsInSetWithinRadiusOfPos(serverGOM.objectSets.sapiens, planObject.pos, gameConstants.planNoCompleteThreatDistance)
                            if allCloseSapiens then
                                for j,objectInfo in ipairs(allCloseSapiens) do
                                    local sapien = serverGOM:getObjectWithID(objectInfo.objectID)
                                    if sapien and serverTribe:getOtherTribeIsThreatening(tribeID, sapien.sharedState.tribeID) then
                                        foundThreatID = sapien.uniqueID
                                        break
                                    end
                                end
                            end

                            if foundThreatID then
                                planObject.sharedState:set("planStates", tribeID, i, "closeThreat", foundThreatID)
                                updateCanCompleteAndSave(planObject, planObject.sharedState, thisPlanState, tribeID, i, false)
                            elseif thisPlanState.closeThreat then
                                planObject.sharedState:remove("planStates", tribeID, i, "closeThreat")
                                updateCanCompleteAndSave(planObject, planObject.sharedState, thisPlanState, tribeID, i, false)
                            end
                        end
...
local function updateImpossibleStateForStorageAvailibilityChange(storageAreaTribeID, objectID, objectTypeIndex, isNewlyAvailable)
    ...
    if object then
        ...
        local planStatesByTribeID = sharedState.planStates
        if planStatesByTribeID then
            local planStates = planStatesByTribeID[storageAreaTribeID]
            if planStates then
                for i,thisPlanState in ipairs(planStates) do
                    if thisPlanState.requiresStorageObjectTypeIndex and thisPlanState.requiresStorageObjectTypeIndex == objectTypeIndex then
                        if ((not thisPlanState.missingStorage) and (not isNewlyAvailable)) or (thisPlanState.missingStorage and isNewlyAvailable) then
                            ...
                            sharedState:set("planStates", storageAreaTribeID, i, "missingStorage", missingStorage)
                            updateCanCompleteAndSave(object, sharedState, thisPlanState, storageAreaTribeID, i, false)
                        end
                    end
                end
            end
                ...
                for i,thisPlanState in ipairs(planStates) do
                    if thisPlanState.planTypeIndex ~= plan.types.haulObject.index then
                        ...
                        elseif thisPlanState.planTypeIndex == plan.types.playInstrument.index then
                            ...
                            if (not constructableTypeIndex) and thisPlanState.discoveryCraftableTypeIndex then --can be safely removed at about 0.7.1, only affected public unstable worlds for 0.7.0, causing a crash on launch
                                constructableTypeIndex = thisPlanState.discoveryCraftableTypeIndex
                                sharedState:set("planStates", tribeID, i, "constructableTypeIndex", constructableTypeIndex)
                            end
...
function planManager:addPlanObject(object)
    ...
    if planStatesByTribeID then
        for tribeID,planStates in pairs(planStatesByTribeID) do
            local threatProximityObjectSetKey = "theatObs_" .. tribeID
            if not serverGOM.objectSets[threatProximityObjectSetKey] then
                serverGOM.objectSets[threatProximityObjectSetKey] = serverGOM:createObjectSet(threatProximityObjectSetKey)
                
                serverGOM:addProximityCallbackForGameObjectsInSet(serverGOM.objectSets[threatProximityObjectSetKey], serverGOM.objectSets.sapiens, gameConstants.planNoCompleteThreatDistance, function(planObjectID, sapienID, distance2, newIsClose)
                    planManager:updateThreatProximity(planObjectID, sapienID, newIsClose)
                end)
            end
           ...
           -- local canComplete = false
            for i,thisPlanState in ipairs(planStates) do
               ...
               --[[ if thisPlanState.manuallyPrioritized then
                ...
                if (not planType.allowCompletionNearThreats) then
                    serverGOM:addObjectToSet(object, serverGOM.objectSets[threatProximityObjectSetKey])
                end
...
function planManager:removePlanObject(object)
    ...
    for tribeID,closeSapiensByObject in pairs(closeThreatSapiensByObjectByTribe) do
        closeSapiensByObject[object.uniqueID] = nil
        if not next(closeSapiensByObject) then
            closeThreatSapiensByObjectByTribe[tribeID] = nil
        end
    end
...
local function hasPlansAvailableForSapien(object, planState, planIndex, sapien)
    ...
    if not allowed then
        --disabled--mj:objectLog(object.uniqueID, "checkSapienHasAbilityForRole returned false")
        return false
    end

    --disabled--mj:objectLog(object.uniqueID, "checkSapienHasAbilityForRole returned true")
...
function planManager:getGameObjectTypeIndexesForRequiredTools(requiredTools, planObject, planState)
...
function planManager:getAttackPlanForSapien(sapien, planObject)
    local planStates = planManager:getPlanStatesForObjectForSapien(planObject, sapien)
    --disabled--mj:objectLog(sapien.uniqueID, "planManager:getAttackPlanForSapien planStates:", planStates, " planObject:", planObject.uniqueID)
    if planStates then
        for i,planState in ipairs(planStates) do
            
            local isAttackPlan = plan.types[planState.planTypeIndex].isAttackPlan or (planState.researchTypeIndex and research.types[planState.researchTypeIndex].isAttackPlan)

            if isAttackPlan then
                local canComplete = true

                if planState.requiredTools then
                    local toolGameObjectTypes = planManager:getGameObjectTypeIndexesForRequiredTools(planState.requiredTools, planObject, planState)
                    local resourceInfo = serverResourceManager:findResourceForSapien(sapien, toolGameObjectTypes, {
                ...
                if canComplete then
                    return planState.planTypeIndex
                end
            end
        end
    end
    --disabled--mj:objectLog(sapien.uniqueID, "planManager:getAttackPlanForSapien returning nil.")
    return nil
end
...
function planManager:reAddBuildOrPlantPlan(tribeID, planTypeIndex, objectID)
    ...
    if buildObject then
        ...
        if constructableTypeIndex then
            ...
            objectState:set("planStates", tribeID, insertIndex, planState)
            finalizeBuildOrPlantPlan(tribeID, buildObject, planState, constructableType)
        end
    end
end
...
function planManager:addBuildOrPlantPlanForRebuild(objectID, tribeID, rebuildConstructableTypeIndex, restrictedResourceObjectTypes, restrictedToolObjectTypes)
    ...
    if buildObject then
        ...
        if restrictedResourceObjectTypes and next(restrictedResourceObjectTypes) then
            buildObject.sharedState:set("restrictedResourceObjectTypes", restrictedResourceObjectTypes)
        end
        if restrictedToolObjectTypes and next(restrictedToolObjectTypes) then
            buildObject.sharedState:set("restrictedToolObjectTypes", restrictedToolObjectTypes)
        end
    end
end
    ...
    if customFunction then
        ...
        if userData.objectOrVertIDs then
            ...
            for i,objectID in ipairs(userData.objectOrVertIDs) do
                ...
                if object then
                    foundObject = true
                    addPlanState(object, tribeID, planTypeIndex, userData.objectTypeIndex, nil, nil, nil, nil, nil, nil, userData.prioritize or (plan.types[planTypeIndex].priorityOffset >= plan.treatMedicalPriorityOffset), nil, userData)
                end
            end
...
function planManager:assignSapienToPlan(tribeID, userData)
    ...
    if userData.sapienID and userData.planObjectID then
            ...
            if planObject then
                ...
                if sharedState then
                    ...
                    if planStatesForThisTribe then
                        ...
                        local function planIsAllowedForThisSapien(planState)
                            if userData.planTypeIndex then
                                if userData.planTypeIndex ~= planState.planTypeIndex then
                                    return false
                                end
                            end

                            if userData.researchTypeIndex then
                                if userData.researchTypeIndex ~= planState.researchTypeIndex then
                                    return false
                                end
                            end
                            if userData.objectTypeIndex then
                                if userData.objectTypeIndex ~= planState.objectTypeIndex then
                                    return false
                                end
                            end
                            
                            if not checkSapienHasAbilityForRole(sapien, planState.requiredSkill, planState) then
                                return false
                            end
                            ...
                            if planState.requiredSkill then 
                                ...
                                if priorityLevel ~= 1 then
                                    if not serverSapien:autoAssignToRole(sapien, planState.requiredSkill) then
                                        return false
                                    end
                                end
                            end
                        ...
                        for planIndex, thisPlanState in ipairs(planStatesForThisTribe) do
                            if planIsAllowedForThisSapien(thisPlanState) then


                                local assignedSapienIDs = mj:cloneTable(sharedState.assignedSapienIDs)
                                if assignedSapienIDs then
                                    local foundAssignedCount = 0
                                    local maxAssignCount = plan.types[thisPlanState.planTypeIndex].maxAssignCount or 1
                                    
                                    for assignedSapienID,planTypeIndexOrTrue in pairs(assignedSapienIDs) do --this order is kind of random but not actually, could do something more clever maybe
                                        ...
                                        if assignedSapien then
                                            ...
                                            if orderQueue and orderQueue[1] then
                                                ...
                                                ((not userData.objectTypeIndex) or orderContext.objectTypeIndex == thisPlanState.objectTypeIndex) then
                                                    ...
                                                    foundAssignedCount = foundAssignedCount + 1
                                                    if foundAssignedCount >= maxAssignCount then
                                ...
                                --the below shouldn't be needed, assignedSapienIDs should cover it
                               --[[ if thisPlanState.manualAssignedSapien and thisPlanState.manualAssignedSapien ~= sapien.uniqueID then
                                    if not plan.types[thisPlanState.planTypeIndex].allowsMu
                                    local assignedSapien = serverGOM:getObjectWithID(thisPlanState.manualAssignedSapien)
                                    if assignedSapien then
                                        serverSapien:cancelAllOrders(assignedSapien, false, true)
                                    end
                                end]]
                                ...
                                sapien.sharedState:set("manualAssignedPlanObjectPreventSleepUntilTime", serverWorld:getWorldTime() + 120) --todo gameConstant
...
function planManager:updateResearchPlansForCraftableDiscoveryResearchCompletion(tribeID, discoveryCraftableTypeIndex)
    local orderedPlans = orderedPlansByTribeID[tribeID]
        ...
        for i, orderedPlan in ipairs(orderedPlans) do
            ...
            if object then
                ...
                if planStatesByTribeID then
                    local planStates = planStatesByTribeID[tribeID]
                    if planStates then
                        for planIndex,thisPlanState in ipairs(planStates) do
                            if thisPlanState.planTypeIndex == plan.types.research.index then
                                local planDiscoveryCraftableTypeIndex = thisPlanState.discoveryCraftableTypeIndex or thisPlanState.constructableTypeIndex
                                if discoveryCraftableTypeIndex == planDiscoveryCraftableTypeIndex then
                                    local constructableTypeIndex = thisPlanState.discoveryCraftableTypeIndex or thisPlanState.constructableTypeIndex
                                    local constructableType = constructable.types[constructableTypeIndex]
                                    if constructableType.skills and constructableType.skills.required then
                                        object.sharedState:remove("planStates", tribeID, planIndex, "researchTypeIndex")
                                        object.sharedState:set("planStates", tribeID, planIndex, "planTypeIndex", plan.types.craft.index)
                                        object.sharedState:set("planStates", tribeID, planIndex, "constructableTypeIndex", discoveryCraftableTypeIndex)
                                        planManager:setRequiredSkillForPlan(tribeID, thisPlanState, object, constructableType.skills.required)
                                    end
                                end
                            end
                        end
...
function planManager:cancelAnyResearchPlans()
   ...
   --[[ if next(researchCraftablePlansToDelayedCancelByTribeID) then
        for tribeID, discoveryCraftableTypeIndexSet in pairs(researchCraftablePlansToDelayedUpdateByTribeID) do
            planManager:doUpdateResearchPlansForCraftableDiscoveryResearchCompletion(tribeID, discoveryCraftableTypeIndexSet)
        end
...
function planManager:modifyFillObjectTypeIndexForAnyPlans(tribeID, userData)
    
    if userData.vertIDs and userData.constructableTypeIndex then
        for vindex, vertID in ipairs(userData.vertIDs) do
            local vert = terrain:getVertWithID(vertID)
            if vert then
                local planObjectID = terrain:getObjectIDForTerrainModificationForVertex(vert)
                if planObjectID then
                    ...
                    if planObject then
                        ...
                        if planStatesByTribeID then
                            ...
                            if planStates then
                                for planIndex,thisPlanState in ipairs(planStates) do
                                    if thisPlanState.planTypeIndex == plan.types.fill.index then

                                        planObject.sharedState:set("planStates", tribeID, planIndex, "constructableTypeIndex", userData.constructableTypeIndex)

                                        local constructableType = constructable.types[userData.constructableTypeIndex]
                                        local requiredItems = serverGOM:getRequiredItemsNotInInventory(planObject, thisPlanState, constructableType.requiredTools, constructableType.requiredResources, userData.constructableTypeIndex)
                                        planManager:updateRequiredResourcesForPlan(tribeID, thisPlanState, planObject, requiredItems)
...
function planManager:checkPlanAvailability(tribeID, userData)
	...
	if userData and userData.plans then
        ...
        --mj:log("userData.plans:", userData.plans)


        if availablePlans then
            
            local expandedUIPlans = {}
            for i, uiPlanInfo in ipairs(userData.plans) do
                if uiPlanInfo.planTypeIndex == plan.types.attack.index or uiPlanInfo.planTypeIndex == plan.types.defend.index then
                    for j,availablePlanInfo in ipairs(availablePlans) do
                        if availablePlanInfo.planGroupTypeIndex == uiPlanInfo.planTypeIndex then
                            local uiPlanCopy = mj:cloneTable(uiPlanInfo)
                            uiPlanCopy.planTypeIndex = availablePlanInfo.planTypeIndex
                            uiPlanCopy.objectTypeIndex = availablePlanInfo.objectTypeIndex
                            uiPlanCopy.researchTypeIndex = availablePlanInfo.researchTypeIndex
                            table.insert(expandedUIPlans, uiPlanCopy)
                        end
                    end
                else
                    table.insert(expandedUIPlans, uiPlanInfo)
                end
            end

            for i, uiPlanInfo in ipairs(expandedUIPlans) do
                ...
                uiPlanInfo.planTypeIndex == plan.types.destroyContents.index then -- bit of a hack, I guess there should be a superset of availablePlansForObjectInfos with hidden plans or something?
                    ...
                    for j,availablePlanInfo in ipairs(availablePlans) do
                        if availablePlanInfo.planTypeIndex == uiPlanInfo.planTypeIndex and 
                        ((not uiPlanInfo.objectTypeIndex) or availablePlanInfo.objectTypeIndex == uiPlanInfo.objectTypeIndex) and 
                        ((not uiPlanInfo.researchTypeIndex) or availablePlanInfo.researchTypeIndex == uiPlanInfo.researchTypeIndex) and 
                        (availablePlanInfo.hasNonQueuedAvailable or availablePlanInfo.hasQueuedPlans) then
                ...
                if foundPlanInfo and (foundPlanInfo.hasNonQueuedAvailable or foundPlanInfo.hasQueuedPlans) then
                    ...
                    if uiPlanInfo.researchTypeIndex then
                        ...
                        if researchType.requiredToolTypeIndex then
                            requiredTools = {researchType.requiredToolTypeIndex}
                           -- mj:log("setting requiredTools a:", requiredTools)
                        end
                    else
                    ...
                    if foundPlanInfo.requirements then --changed 0.4 to override the researchType with the planInfo from planHelper, as it gives more contextural control 
                        ...
                        if foundPlanInfo.requirements.toolTypeIndex then
                            requiredTools = {foundPlanInfo.requirements.toolTypeIndex}
                            --mj:log("setting requiredTools b:", requiredTools)
                        end
                    end
                    ...
                    if (not plan.types[uiPlanInfo.planTypeIndex].allowCompletionNearThreats) and (not (researchType and researchType.allowCompletionNearThreats)) then
                        local allCloseSapiens = serverGOM:getGameObjectsInSetWithinRadiusOfPos(serverGOM.objectSets.sapiens, objectsOrVerts[1].pos, gameConstants.planNoCompleteThreatDistance)

                        local checkedTribes = {}
                        for j,objectInfo in ipairs(allCloseSapiens) do
                            local sapien = serverGOM:getObjectWithID(objectInfo.objectID)
                            if sapien then
                                local otherTribeID = sapien.sharedState.tribeID
                                if not checkedTribes[otherTribeID] then
                                    checkedTribes[otherTribeID] = true
                                    if serverTribe:getOtherTribeIsThreatening(tribeID, otherTribeID) then
                                        addProblem("closeThreat", true)
                                        break
                                    end
                                end
                            end
                    ...
                    if constructableTypeIndexForRequiredStuff then
                        ...
                        if not requiredTools then
                            ...
                            --mj:log("setting requiredTools c:", requiredTools)
                        end
                    end
